...
/**
 * TypeScript Script: Multihop Swap X → Y → Z (Single Transaction)
 * Based on IDL: MultihopSwap (discriminant: 4)
 * Args: amount_in (u64), minimum_amount_out (u64)
 */
async function multihopSwapXYZ() {
  try {
    console.log("🚀 TypeScript Script: Multihop Swap X → Y → Z (Single Transaction)...");
    ...
    fs.writeFileSync("multihop-swap-x-y-z-single-info.json", JSON.stringify(multihopSwapInfo, null, 2));
    console.log("\n💾 Multihop swap info saved to multihop-swap-x-y-z-single-info.json");
