...
/**
 * TypeScript Script: Multihop Swap with Path S → T → U
...
async function multihopSwapWithPath() {
  try {
    console.log("🚀 TypeScript Script: Multihop Swap with Path S → T → U...");
    ...
    const amountIn = 8_000_000_000; // 8 Token S
    const minimumAmountOut = 1; // Minimum 1 unit of Token U
    const tokenPath = [TOKEN_S_MINT, TOKEN_T_MINT, TOKEN_U_MINT]; // S → T → U
    ...
    // Prepare accounts for MultihopSwapWithPath (matching working JavaScript script order)
    const accounts = [
    ...
    // Instruction data (Borsh: MultihopSwapWithPath { amount_in, minimum_amount_out, token_path })
    // For vec<publicKey>, we need to serialize: length (u32) + publicKeys
    const pathLength = tokenPath.length;
    const pathData = Buffer.alloc(4 + (pathLength * 32)); // 4 bytes for length + 32 bytes per pubkey
    pathData.writeUInt32LE(pathLength, 0);
    for (let i = 0; i < pathLength; i++) {
      tokenPath[i].toBuffer().copy(pathData, 4 + (i * 32));
    }
    
    const data = Buffer.alloc(1 + 8 + 8 + pathData.length); // discriminator + amount_in + minimum_amount_out + path_data
    data.writeUInt8(5, 0); // MultihopSwapWithPath discriminator
    data.writeBigUInt64LE(BigInt(amountIn), 1);
    data.writeBigUInt64LE(BigInt(minimumAmountOut), 9);
    pathData.copy(data, 17);
    
    console.log(`\n📝 Instruction data: ${data.toString('hex')}`);

    // Add MultihopSwapWithPath instruction
    console.log("📝 Adding MultihopSwapWithPath instruction...");
    transaction.add({
      keys: accounts,
    ...
    console.log(`✅ Multihop swap with path S → T → U completed successfully!`);
    ...
    console.log(`\n📈 Multihop Swap with Path Results:`);
    ...
    fs.writeFileSync("multihop-swap-with-path-results.json", JSON.stringify(swapInfo, null, 2));
    console.log("\n💾 Multihop swap with path results saved to multihop-swap-with-path-results.json");

  } catch (error) {
    console.error("❌ Error in multihop swap with path S → T → U:", error);
    throw error;
  }
}
