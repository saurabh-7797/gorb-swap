...
/**
 * Huge Multihop Swap X to Z
 * Function: hugeMultihopXToZ()
 * Purpose: Performs a huge multihop swap from Token X to Token Z via Token Y (0.1 tokens)
 */
async function hugeMultihopXToZ() {
  try {
    console.log("🚀 Huge Multihop Swap: X → Y → Z (0.1 tokens)...");
    
    // Load all token and pool info
    ...
    console.log("\n📊 Balances BEFORE Huge Multihop Swap:");
    ...
    // Huge multihop swap parameters
    const amountIn = 100_000_000; // 0.1 tokens (much smaller amount to avoid intermediate balance issues)
    const minimumAmountOut = 0; // 0 Token Z (no slippage protection to allow the swap)
    const tokenPath = [TOKEN_X_MINT, TOKEN_Y_MINT, TOKEN_Z_MINT];
    
    console.log(`\n🛣️  Huge Multihop Swap Parameters:`);
    ...
    // Check if intermediate token account exists, if not create it
    try {
      await getAccount(connection, intermediateTokenY, "confirmed", SPL_TOKEN_PROGRAM_ID);
      console.log(`✅ Intermediate Token Y account exists: ${intermediateTokenY.toString()}`);
    } catch (error) {
      console.log(`⚠️  Intermediate Token Y account doesn't exist, will be created during transaction`);
    }
    ...
    console.log("📝 Adding Huge MultihopSwap instruction...");
    ...
    console.log("✅ Huge Multihop Swap transaction successful!");
    console.log(`Transaction signature: ${sig}`);
    console.log(`View on GorbScan: https://gorbscan.com/tx/${sig}`);
    
    // Check balances after multihop swap
    console.log("\n📊 Balances AFTER Huge Multihop Swap:");
    ...
    // Save huge multihop swap results
    const multihopResults = {
      swapType: "Huge Multihop Swap X to Z",
      amountIn: amountIn,
      ...
      path: [TOKEN_X_MINT.toString(), TOKEN_Y_MINT.toString(), TOKEN_Z_MINT.toString()],
      transactionSignature: sig
    };
    
    fs.writeFileSync('huge-multihop-x-to-z-results.json', JSON.stringify(multihopResults, null, 2));
    console.log("💾 Huge multihop swap results saved to huge-multihop-x-to-z-results.json");
    
    console.log(`\n🎉 Huge Multihop Swap X→Y→Z Test Completed Successfully!`);
    ...
    console.error("❌ Error in Huge Multihop Swap:", error.message);
    if (error.logs) {
...
hugeMultihopXToZ().catch(console.error);
