...
/**
 * Step 10: Remove Liquidity from Pool Y-Z
 * Function: removeLiquidityYZ()
 * Purpose: Removes liquidity from the existing Pool Y-Z by burning LP tokens
 */
async function removeLiquidityYZ() {
  try {
    console.log("🚀 Step 10: Removing Liquidity from Pool Y-Z...");
    ...
    console.log(`LP Tokens Burned: ${formatTokenAmount(-lpChange)} (${-lpChange} raw)`);
    console.log(`Total Value Recovered: ${formatTokenAmount(tokenYChange + tokenZChange)} tokens`);
    
    // Update pool info with liquidity removal
    const updatedPoolInfo = {
      ...poolInfo,
      removedLiquidityY: tokenYChange,
      removedLiquidityZ: tokenZChange,
...
removeLiquidityYZ().catch(console.error);
