...
/**
 * Massive Swap Y to Z
 * Function: massiveSwapYToZ()
 * Purpose: Performs a massive swap from Token Y to Token Z (10.0 tokens)
 */
async function massiveSwapYToZ() {
  try {
    console.log("🚀 Massive Swap: Y → Z (10.0 tokens)...");
    ...
    // Massive swap parameters
    const amountIn = 10_000_000_000; // 10.0 tokens
    const directionAToB = true; // Y to Z
    
    console.log(`\n🔄 Massive Swap Parameters:`);
    console.log(`Amount In: ${formatTokenAmount(amountIn)} Token Y`);
    console.log(`Direction: Y → Z`);
    ...
    console.log("📝 Adding Massive Swap instruction...");
    ...
    console.log("✅ Massive Swap transaction successful!");
    console.log(`Transaction signature: ${sig}`);
    console.log(`View on GorbScan: https://gorbscan.com/tx/${sig}`);
    
    // Check balances after swap
    console.log("\n📊 Balances AFTER Massive Swap:");
    ...
    console.log(`\n💰 Massive Swap Summary:`);
    console.log(`Tokens Swapped:`);
    console.log(`  - Token Y Spent: ${formatTokenAmount(-tokenYChange)} (${-tokenYChange} raw)`);
    console.log(`  - Token Z Received: ${formatTokenAmount(tokenZChange)} (${tokenZChange} raw)`);
    console.log(`Effective Exchange Rate: 1 Token Y = ${formatTokenAmount(tokenZChange / -tokenYChange)} Token Z`);
    
    // Save massive swap results
    const swapResults = {
      swapType: "Massive Swap Y to Z",
      amountIn: amountIn,
      amountOut: tokenZChange,
      tokenYSpent: -tokenYChange,
      tokenZReceived: tokenZChange,
      exchangeRate: tokenZChange / -tokenYChange,
      transactionSignature: sig
    };
    
    fs.writeFileSync('massive-swap-y-to-z-results.json', JSON.stringify(swapResults, null, 2));
    console.log("💾 Massive swap results saved to massive-swap-y-to-z-results.json");
    
    return swapResults;
    
  } catch (error) {
    console.error("❌ Error in Massive Swap:", error.message);
    if (error.logs) {
...
massiveSwapYToZ().catch(console.error);
