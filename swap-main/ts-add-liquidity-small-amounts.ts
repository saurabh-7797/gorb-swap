...
/**
 * Add Small Amount of Liquidity to A-Wrapped SOL Pool
 * This adds a small amount of liquidity to test the functionality
 */
async function addSmallLiquidity() {
  try {
    console.log("🚀 Adding Small Liquidity to A-Wrapped SOL Pool...");
    ...
    // 3. Use smaller amounts - only what we have available
    const amountA = Math.min(100_000_000, balanceTokenABefore); // 0.1 Token A or less
    const amountWrappedSOL = Math.min(100_000_000, balanceWrappedSOLBefore); // 0.1 Wrapped SOL or less
    ...
    if (amountA === 0 || amountWrappedSOL === 0) {
...
// Run the function
addSmallLiquidity().catch(console.error);
